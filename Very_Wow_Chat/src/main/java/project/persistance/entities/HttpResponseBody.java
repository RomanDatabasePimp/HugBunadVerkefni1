...
public class HttpResponseBody {
  ...
  /* We want to have somekind of way to denote right away if the response is a successful or not
   * when u get a response it will bed wrapped in BadResp or GoodResp this way in the api-communicator
   * this style along with Http status codes gives us a very good way of handling responses late down the line */
  private JSONObject errors_response;
  private JSONObject successful_response;
  
  /* A response can consist of many thing this Array can store many individual things 
   * so maybe if u only want to send one response it is possible*/
  private ArrayList<JSONObject> error;    // container for  errors
  private ArrayList<JSONObject> response; // container for  successes
  
  /* Sometimes you want to return a obj that has a list of objects that denote different
   * things, this is also possible ! */
   ...
   *  For  : arrayKey(STRING)  - ID of the array you want to add this into
   *         newobj(JSONObject)- just a jsonobject 
  ...
  public String getErrorResponse()  {
	if(this.many_in_one_erros.length() > 0 ) { this.error.add(this.many_in_one_erros);}
	return this.errors_response.put("BadResp", this.error).toString();
  }
  public String getSuccessResponse(){
	if(this.many_in_one_succ.length() > 0 ) { this.response.add(this.many_in_one_succ);}
